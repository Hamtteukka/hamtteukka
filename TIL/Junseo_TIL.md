...
1. 소스 코드 작성 : 개발자가 컴퓨터가 이해할 수 있는 언어로 소스 코드를 작성한다.
2. 컴파일러 : 소스 코드를 기계어 코드로 변환한다. 컴파일러는 소스 코드를 분석하고 최적화 하여 중간 코드나 직접 실행 가능한 기계 코드를 생성한다.
3. 링커 : 컴파일 된 파일들을 하나로 묶어 실행 가능한 파일을 생성한다. 이 과정에서 외부 라이브러리나 다른 모듈과의 연결이 이루어진다.
4. 로더 : 링크된 실행 파일을 메모리로 적재하고, 실행을 시작한다. 로더는 필요한 라이브러리와 함께 프로세스의 주소 공간에 적재한다. 
...
전체 소스 코드를 미리 기계어로 번역하여 실행 파일을 생성한다. 실행 속도가 빠르지만, 개발 과정에서의 유연성은 낮다. (ex : C, C++)

**인터프리터 언어**

소스 코드를 한 줄씩 읽고 즉시 실행한다. 개발 및 테스트가 용이하지만, 실행 속도는 상대적으로 느릴 수 있다. (ex : Python)
...
자바의 경우, 코드를 실행하기 위해서는 바이트 코드로 컴파일 하는 과정과 바이트 코드를 인터프리트 하는 과정을 거쳐야 하기 때문에 컴파일 과정만 필요한 다른 프로그래밍 언어보다 느리다. 

이러한 성능 문제를 개선하기 위해 나온 것이 JIT 컴파일러이다.

JIT컴파일러는 실행 시점에서는 인터프리터와 같이 기계어 코드를 생성하면서 해당 코드가 컴파일 대상이 되면 컴파일 하고 그 코드를 캐싱한다. JIT 컴파일은 코드가 실행되는 과정에서 실시간으로 일어나며, 전체 코드의 필요한 부분만 변환한다. 기계어로 변환된 코드는 캐시에 저장되기 때문에 재사용 시 재 컴파일 할 필요가 없다. 


> ❗ **본인이 사용하는 언어는, 어떤식으로 컴파일 및 실행되는지 설명해 주세요.**

[[Java] 컴파일 과정 | 👨🏻‍💻 Tech Interview](https://gyoogle.dev/blog/computer-language/Java/%EC%BB%B4%ED%8C%8C%EC%9D%BC%20%EA%B3%BC%EC%A0%95.html)

**[ JAVA ]**

1. 개발자가 자바 소스 코드를 작성한다.
2. 자바 컴파일러가 자바 소스 파일을 컴파일 한다. 이 때 나오는 파일은 자바 바이트 코드 파일로, 아직 컴퓨터가 읽을 수 없는 JVM이 이해할 수 있는 코드이다. 
3. 컴파일 된 바이트 코드를 JVM의 클래스 로더에게 전달한다. 
4. 클래스 로더는 동적 로딩을 통해 필요한 클래스들을 로딩 및 링크해서 런타임 데이터 영역, JVM의 메모리에 올린다.
...
- API 요청 횟수가 제한 장치에 정의된 임계치(threshold)를 넘어서면 추가로 도달한 모든 호출은 처리가 중단 된다.
...
직관적으로 보자면 이 장치는 클라이언트에도 둘 수 있고, 서버 측에도 둘 수 있다. 

- 클라이언트 측에 두면 : 일반적으로 클라이언트는 처리율 제한을 안정적으로 걸 수 있는 장소가 못 된다. 클라이언트 요청은 쉽게 위변조가 가능해서다.
...
토큰 버킷은 지정된 용량을 갖는 컨테이너이다. 이 버킷에는 사전 설정된 양의 토큰이 주기적으로 채워진다. 토큰이 꽉 찬 버킷에는 더 이상의 토큰은 추가되지 않는다. 버킷이 가득 차면 추가로 공급된 토큰은 버려진다. (overflow)

각 요청은 처리될 때 마다 하나의 토큰을 사용한다. 요청이 도착하면 버킷에 충분한 토큰이 있는지 검사하게 된다. 

- 충분한 토큰이 있는 경우 : 버킷에서 하나의 토큰을 꺼낸 후 요청을 시스템에 전달한다.
...
- 구현이 쉽다
- 메모리 사용 측면에서도 효율적이다.
- 짧은 시간에 집중되는 트래픽도 처리 가능하다. 버킷에 남은 토큰이 있기만 하면 요청은 시스템에 전달 될 것이다.
...
토큰 버킷 알고리즘과 비슷하지만 **요청 처리율**이 고정되어 있다는 점이 다르다. 
...
- 처리율 : 지정된 시간당 몇 개의 항목을 처리할 지 지정하는 값이다. 보통 초 단위로 표현된다.
...
- 타임라인을 고정된 간격 윈도로 나누고, 각 윈도마다 카운터를 붙인다.
- 요청이 접수 될 때마다 이 카운터의 값은 1씩 증가한다.
- 이 카운터의 값이 사전에 설정된 임계치에 도달하면 새로운 요청은 새 윈도가 열릴 때 까지 버려진다.

이 알고리즘의 가장 큰 문제는 **윈도의 경계 부근에 순간적으로 많은 트래픽이 집중될 경우 윈도에 할당된 양보다 더 많은 요청이 처리될 수 있다**는 것이다. 
...
- 요청의 타임 스탬프를 추적한다. 타임 스탬프 데이터는 보통 레디스의 정렬 집합 같은 캐시에 보관한다.
...
**고정 윈도 카운터 알고리즘**과 **이동 윈도 로깅 알고리즘**을 결합한 것이다. 
...
처리율 제한 알고리즘의 기본 아이디어는 단순하다. 얼마나 많은 요청이 접수 되었는지를 추적할 수 있는 카운터를 추적 대상별로 두고 이 카운터의 값이 어떤 한도를 넘어서면 한도를 넘어 도착한 요청은 거부하는 것이다. 

**이 카운터는 어디 보관할 것인가 ?**

데이터 베이스는 디스크 접근 때문에 느리니까 사용하면 안된다. 메모리 상에서 동작하는 캐시가 바람직 한데, 빠른데다 시간에 기반한 만료 정책을 지원하기 때문이다. 
...
클라이언트는 자기 요청이 처리율 제한에 걸리고 있는지를 어떻게 감지할 수 있나? 자기 요청이 처리율 제한에 걸리기까지 얼마나 많은 요청을 보낼 수 있는지는 어떻게 알 수 있나?
...
경쟁 조건을 해결하는 가장 널리 알려진 해결책은 락이다. 하지만 락은 성능을 상당히 떨어트린다는 문제가 있다. 락 대신 해결할 수 있는 방법은 다음 두 가지가 있다.

- 루아 스크립트
- 정렬 집합이라 불리는 레디스 자료 구조를 쓰는 것이다.

### 동기화 이슈

**sticky session**

- 같은 클라이언트로부터의 요청은 항상 같은 처리율 제한 장치로 보내게 함.
- 규모 면에서 확장 가능하지도 않고 유연하지도 않음
...
[python.org](http://python.org) 에서 배포되고 있는 Python을 의미한다. 가장 널리 사용되고, 안정적인 파이썬 구현체이다.

**PyPy**

CPython과 동일한 파이썬 코드를 실행하는 **JIT 컴파일러**가 포함된 구현체이다. 

**Jython**

Python을 구현할 때 JVM을 사용한 Python

**실행 과정의 차이**

- **CPython**은 파이썬 코드를 바이트코드로 변환한 후, 가상 머신에서 실행한다.
- **PyPy**는 파이썬 코드를 바이트코드로 변환 후, JIT 컴파일러가 실행 중에 최적화된 네이티브 코드를 생성해 실행한다.
- **Jython**은 파이썬 코드를 **Java 바이트코드**로 변환하여 JVM에서 실행한다.


> ❗ **우리는 흔히 fork(), exec() 시스템 콜을 사용하여 프로세스를 적재할 수 있다고 배웠습니다. 로더의 역할은 이 시스템 콜과 상관있는 걸까요? 아니면 다른 방식으로 프로세스를 적재할 수 있는 건가요?**


fork() 는 현재 프로세스의 복사본을 만들어 새로운 프로세스를 생성한다. 이 때, 새로운 프로세스는 부모 프로세스와 동일한 코드를 가지지만 독립적인 메모리 공간을 갖게 된다. 

exex() 는 현재 프로세스의 메모리 공간을 새로운 프로그램의 코드로 교체한다. 이 때, 부모 프로세스의 메모리 내용을 새로운 프로그램으로 덮어쓰고 그 프로그램을 실행한다.

**로더의 역할**

로더는 실행 파일을 메모리에 적재하는 역할을 한다. 실행하려는 프로그램 파일을 읽어 들여 필요한 메모리 영역에 로딩 후 실행을 준비한다. 

### 요약

- `fork()`는 프로세스를 생성하는 시스템 콜이고,
- `exec()`는 프로세스를 새로운 프로그램으로 교체하는 시스템 콜이다.
- 로더는 실행 파일을 메모리에 적재하고 실행 준비를 하는 시스템의 부분이다.

즉, `fork()`와 `exec()`는 프로세스 생성 및 실행을 위한 시스템 콜이라면, **로더는 그 실행 파일을 메모리에 올려서 실행할 수 있게 해주는 역할**을 한다. 
...
[[운영체제] IPC Inter-Process Communication](https://yaelimeee.tistory.com/56)

프로세스는 다른 프로세스와 통신 여부에 따라 독립 프로세스와 협력 프로세스로 나눌 수 있다. IPC 란 협력 프로세스 사이에서 다른 프로세스에게 정보를 주거나 받을 수 있는 운영체제의 모델이다. 

IPC에는 **Pipe, Named Pipe, Message Queue,  Memory Map, Shared Memory**와 **Message Passing** 방식이 존재한다. 


> ❗ **Shared Memory가 무엇이며, 사용할 때 유의해야 할 점에 대해 설명해 주세요.**


[공유 메모리 (Shared Memory) 이해하기](https://velog.io/@k1505ww/%EA%B3%B5%EC%9C%A0-%EB%A9%94%EB%AA%A8%EB%A6%AC-Shared-Memory)

Shared Memory는 여러 프로세스가 접근할 수 있는 공유 메모리 영역을 구축하여 정보를 주고 받는다.

다만, 여러 프로세스가 공유 메모리 영역 내 하나의 변수에 접근하면 여러 문제가 발생할 수 있다. 따라서 사용할 때 서로 공유되는 영역에 대한 동기화에 유의해야 한다. 

이런 데이터 접근을 동기화 하여 데이터 일관성을 유지하기 위한 방안으로 Semaphore가 사용된다. 임계 영역을 설정하고, 프로세스가 이 영역에 들어갈 수 있도록 허용 하는 방식으로 동작한다. 
...
[20. 프로세스 - IPC 기법(메세지 큐)](https://devraphy.tistory.com/173)

메시지 큐는 단방향이 아니다. 단방향 및 양방향 통신을 모두 지원할 수 있다. 
